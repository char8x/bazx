import { BazxExec, BazxExecConfig } from "./provider.ts";
...
const createStdoutLogger = (context: BazxOptions) =>
  context.log
  ...
  return new WritableStream<Uint8Array>({
    async write(chunk) {
      const buffer = new Uint8Array(
        redBuffer.byteLength + chunk.byteLength + resetBuffer.byteLength,
      );
...
class Command implements Body, PromiseLike<Response> {
  ...
  get body(): ReadableStream<Uint8Array> {
    ...
    this.stdout
      ...
      .catch((err) =>
        buffer.writable.abort(err).catch(
  ...
  pipe(
    commandOrTransformStream: Command | TransformStream<Uint8Array, Uint8Array>,
  ): Command;
  pipe(
    commandOrTransformStream: Command | TransformStream<Uint8Array, Uint8Array>,
  ) {
        ...
        async ({ cwd, env = {}, stdin, stdout, stderr, signal } = {}) => {
          ...
          const [, result] = await Promise.all([
            this.#fetch({
              ...
              stderr: !writer ? undefined : mergedWritableStream(
                writer,
                () => (--left || writer.close()),
              ),
              signal,
...
function rejectOnNonZeroExitCode(response: Response) {
  if (!response.ok) {
    return Promise.reject(
      Object.assign(
        new Error(
          `Command ${response.url} exited with code ${response.status}`,
        ),
        { response },
      ),
    );
...
export function createBazx(exec: BazxExec, options: BazxOptions = {}): $ {
  function $(xs: TemplateStringsArray, ...args: any[]) {
    ...
    return new Command(
      ...
      (colors) =>
        colors
          ? `\x1B[32m${parsed[0]}\x1B[0m ${
            parsed.slice(1).map((x) => `\x1B[4m${x}\x1B[0m`).join(" ")
          }`.trim()
          : parsed.join(" "),
